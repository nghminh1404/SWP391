...
public class UserFilter extends HttpServlet {
    ...
    throws ServletException, IOException {
        ...
        String raw_setting = request.getParameter("setting_id");
        
        //validate
        Boolean status = (raw_status!=null && raw_status.length()>0 && !raw_status.equals("both"))
                ?raw_status.equals("activate"):null;
        Integer settingid =(raw_setting !=null && raw_setting.length() >0
                && !raw_setting.equals("-1")
                )?new Integer(raw_setting):null;
        SettingDBContext dbSetting = new SettingDBContext();
        UserDBContext dbUser = new UserDBContext(); 
        request.setAttribute("settings", dbSetting.list());
        ArrayList<User> u = dbUser.filter(settingid, status);
        request.setAttribute("users", dbUser.filter(settingid, status));    
